...
export class LxDeviceStore {
  ...
  async clearAll(): Promise<void> {
    await this.runExclusive(async () => {
      this.cache = new Map();
      await songloft.persistentStorage.set(LX_SYNC_DEVICES_KEY, JSON.stringify({}));
    });
  }
  ...
  private async ensureUnlocked(): Promise<Map<string, LxClientKeyInfo>> {
    ...
    const raw = safeParse(await songloft.persistentStorage.get(LX_SYNC_DEVICES_KEY));
  ...
  private async persistUnlocked(map: Map<string, LxClientKeyInfo>): Promise<void> {
    ...
    await songloft.persistentStorage.set(LX_SYNC_DEVICES_KEY, JSON.stringify(obj));
    this.cache = map;
  }
}
